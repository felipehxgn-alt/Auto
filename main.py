...
EDICAO:
  - Produto (capa + angulos): remove fundo (PhotoRoom), centraliza em canvas
    1200x1200 com margem de 0,15 (15% de respiro de cada lado). Nada do
    produto pode ser cortado.
  - Logo da marca: tamanho padronizado (fixo) em TODAS as fotos, canto
    superior esquerdo, margem de 0,15. Se a area do logo for coberta pela
    peca, o logo fica ATRAS (peca sempre na frente, nunca cortada).
  - Caixa: tambem tem o fundo removido.
  - Logo vem da pasta de logos no Dropbox, nome do arquivo = MARCA em
    MAIUSCULO (ex: ELRING.png).

CONTROLE DE QUALIDADE (prioridade maxima - nunca anunciar peca errada):
  - Se o SKU/marca nao forem lidos com confianca na foto da caixa, OU o
    logo da marca nao for encontrado, o lote NAO e publicado
    automaticamente: vai para "<SOURCE>/_REVISAR" e dispara alerta.
  - Erros tecnicos tambem disparam alerta e o lote fica intocado na
    entrada (nao se move nem se perde).

Segredos esperados (GitHub Secrets -> variaveis de ambiente):
  DROPBOX_ACCESS_TOKEN, OPENAI_API_KEY, PHOTOROOM_API_KEY

Variaveis opcionais (tem default, pode sobrescrever se quiser):
...
cadastrados, ver aviso no fim da conversa):
  GMAIL_USER, GMAIL_APP_PASSWORD, ALERT_EMAIL_TO

ATENCAO - TOKEN DO DROPBOX:
  Um "Access token" gerado direto no App Console expira em poucas horas.
  Como esse robo roda sozinho, sem alguem clicando de novo, ele precisa
  de um REFRESH TOKEN (nao expira). Ver passo a passo pendente no fim da
  conversa antes de considerar isso definitivo.
"""
...
CANVAS_SIZE = 1200
MARGEM_RATIO = 0.15          # respiro do produto, de cada lado
LOGO_WIDTH_RATIO = 0.15      # largura do logo = 15% do canvas, sempre fixa
LOGO_MARGEM_RATIO = 0.15     # distancia do logo ate as bordas (topo-esquerda)
...
def dbx_listar_pasta(path):
    """Lista arquivos de uma pasta do Dropbox (nao recursivo), ordenados
    pela data de modificacao (proxy da ordem de captura/upload)."""
    entradas = []
    resp = requests.post(
    ...
    if resp.status_code == 409:
        return []  # pasta ainda nao existe / vazia
...
def dbx_buscar_logo(marca):
    """Logo salvo com nome = MARCA em maiusculo (ex: ELRING.png)."""
...
def identificar_sku_marca(imagem_bytes):
    """Retorna (sku, marca, confiante:bool). Nunca inventa valor - se o
    modelo nao tiver certeza, confiante=False e o lote vai pra revisao."""
...
def remover_fundo(imagem_bytes):
    ...
    resp.raise_for_status()
    return resp.content  # PNG com fundo transparente
...
def compor_produto_em_canvas(imagem_bytes_sem_fundo, logo_bytes):
    """Centraliza produto (fundo ja removido) em canvas 1200x1200 com
    margem de 0,15 de cada lado. Logo fixo no canto superior esquerdo,
    tamanho padronizado, sempre ATRAS do produto (produto nunca cortado)."""
    ...
    # 1) logo primeiro (camada de tras)
    if logo_bytes:
        canvas = colar_logo(canvas, logo_bytes)

    # 2) produto por cima (nunca fica atras, nunca e cortado)
...
def colar_logo(canvas_rgba, logo_bytes):
    ...
    largura_alvo = int(canvas_rgba.width * LOGO_WIDTH_RATIO)  # tamanho fixo/padronizado
    ...
    pos = (margem, margem)  # canto superior esquerdo
...
def processar_lote(lote):
    ...
    # --- Move originais pra fora da entrada bruta ---
    for arq in lote:
...
def main():
    ...
    # ignora o que ja estiver dentro de _PROCESSADOS / _REVISAR
    arquivos = [
        f for f in arquivos
